    ...
    const getStatusBadge = (status) => {
        if (isDark) {
            const badges = {
                ...
                investigating: 'bg-blue-900/50 text-blue-200',
        ...
        return lightBadges[status] || 'bg-gray-100 text-gray-800';
    };
        ...
        <AuthenticatedLayout
            header={
                <div className="flex justify-between items-center">
                    <h2 className={`font-semibold text-xl leading-tight ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
                        My Reports
                    </h2>
        ...
        >
            ...
            <div className={`relative py-12 overflow-hidden ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
                ...
                <div className="relative z-20 max-w-7xl mx-auto sm:px-6 lg:px-8">
                    {/* Safety Information */}
                    <div className={`rounded-xl p-8 mb-8 shadow-lg ${isDark ? 'bg-gray-800 border border-gray-600' : 'bg-white border border-gray-200'}`}>
                        <div className="flex">
                            <div className="flex-shrink-0">
                                <span className={`text-xl ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>🛡️</span>
                            </div>
                            <div className="ml-3">
                                <h3 className={`text-sm font-medium ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>
                    ...
                    <div className={`backdrop-blur-sm overflow-hidden shadow-lg sm:rounded-xl mb-8 ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
                        <div className="p-8">
                    ...
                    {!reports?.data?.length ? (
                        <div className={`backdrop-blur-sm overflow-hidden shadow-lg sm:rounded-xl ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
                            <div className="p-16 text-center">
                                ...
                                <div className="space-y-4">
                                    <div className={`rounded-xl p-6 shadow-md ${isDark ? 'bg-gray-700 border border-gray-600' : 'bg-gray-50 border border-gray-200'}`}>
                                        <h4 className={`font-medium mb-2 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>When to report:</h4>
                        ...
                        <div className="space-y-6">
                            {reports.data.map((report) => (
                                <div key={report.id} className={`backdrop-blur-sm overflow-hidden shadow-lg sm:rounded-xl ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
                                    <div className="p-8">
                                        <div className="flex items-start justify-between">
                                            <div className="flex-1">
                                                ...
                                                {report.project && (
                                                    <div className={`mb-4 p-4 rounded-xl ${isDark ? 'bg-gray-700 border border-gray-600' : 'bg-gray-50 border border-gray-200'}`}>
                                                        <div className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Related Project:</div>
                                                ...
                                                {report.evidence && report.evidence.length > 0 && (
                                                    <div className="mb-4">
                                                        <div className={`text-sm mb-2 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Evidence Provided:</div>
                                                ...
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <div className="flex items-center space-x-2">
                                                        <Link
                                                            ...
                                                            className={`inline-flex items-center px-4 py-2 shadow-lg text-sm font-semibold rounded-xl transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 hover:shadow-xl transform hover:scale-105 ${isDark ? 'border border-gray-600 text-gray-300 bg-gray-800 hover:bg-gray-700' : 'border border-gray-300 text-gray-700 bg-white hover:bg-gray-50'}`}
                                                        >
                                                            View Details
                                                        </Link>
                                                    </div>
                                                ...
                                                {report.admin_notes && (
                                                    <div className={`mt-4 rounded-md p-3 ${isDark ? 'bg-blue-900/50 border border-blue-700' : 'bg-blue-50 border border-blue-200'}`}>
                                                        <div className={`text-sm font-medium mb-1 ${isDark ? 'text-blue-200' : 'text-blue-800'}`}>Admin Response:</div>
                                                        <div className={`text-sm ${isDark ? 'text-blue-300' : 'text-blue-700'}`}>{report.admin_notes}</div>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                            ...
                            {reports?.links && reports.links.length > 3 && (
                                <div className={`px-4 py-3 flex items-center justify-between sm:px-6 rounded-lg shadow-sm ${isDark ? 'bg-gray-800 border-t border-gray-700' : 'bg-white border-t border-gray-200'}`}>
                                    <div className="flex-1 flex justify-between sm:hidden">
                                        {reports.prev_page_url && (
                                            <Link
                                                href={reports.prev_page_url}
                                                className={`relative inline-flex items-center px-4 py-2 text-sm font-medium rounded-md ${isDark ? 'border border-gray-600 text-gray-300 bg-gray-800 hover:bg-gray-700' : 'border border-gray-300 text-gray-700 bg-white hover:bg-gray-50'}`}
                                            >
                                                Previous
                                            </Link>
                                        )}
                                        {reports.next_page_url && (
                                            <Link
                                                ...
                                                className={`ml-3 relative inline-flex items-center px-4 py-2 text-sm font-medium rounded-md ${isDark ? 'border border-gray-600 text-gray-300 bg-gray-800 hover:bg-gray-700' : 'border border-gray-300 text-gray-700 bg-white hover:bg-gray-50'}`}
                                            >
                                                Next
                                            </Link>
                                        )}
                                    ...
                                    <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                        <div>
                                            <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                                                Showing <span className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{reports.from}</span> to{' '}
                                                <span className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{reports.to}</span> of{' '}
                                                <span className={`font-medium ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>{reports.total}</span> results
                                            </p>
                                        </div>
                                        <div>
                    ...
                    {/* Report Guidelines */}
                    <div className={`mt-8 backdrop-blur-sm overflow-hidden shadow-lg sm:rounded-xl ${isDark ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
                        <div className="p-8">
                            <h3 className={`text-lg font-semibold mb-4 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>📋 Reporting Guidelines</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <h4 className={`font-medium mb-3 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>What happens after you report:</h4>
                                    <ol className={`text-sm space-y-2 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                                        <li className="flex items-start">
                                            <span className={`mr-2 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>1.</span>
                                            <span>Your report is immediately flagged for review</span>
                                        </li>
                                        <li className="flex items-start">
                                            <span className={`mr-2 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>2.</span>
                                            <span>Our team investigates the reported behavior</span>
                                        </li>
                                        <li className="flex items-start">
                                            <span className={`mr-2 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>3.</span>
                                            <span>Appropriate action is taken if violations are found</span>
                                        </li>
                                        <li className="flex items-start">
                                            <span className={`mr-2 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>4.</span>
                                            <span>You receive an update on the resolution</span>
                                        </li>
                                    </ol>
                                </div>
                                <div>
                                    <h4 className={`font-medium mb-3 ${isDark ? 'text-gray-100' : 'text-gray-900'}`}>Tips for effective reporting:</h4>
